...
const AttendanceForm = ({ className = "" }) => {
  const dispatch = useDispatch();
  const { checkInTime, checkOutTime, totalHours, date } =
    useSelector((state) => state.attendance);
  const [shiftTime, setShiftTimeState] = useState("");
  const [location, setLocation] = useState("");
  const [isCheckedIn, setIsCheckedIn] = useState(false);
  ...
  <div className={`emp-attendance-form-container ${className}`}>
    ...
    <div className="emp-main-container-attendance">
      <div className="emp-time-container">
        ...
        <div className="work-hours">
          <div className="emp-check-in-time">
              Check-in Time: {checkInTime ? formatTime(checkInTime, date) : 'N/A'}
          </div>
          <br />
          <div>
              Check-out Time: {checkOutTime ? formatTime(checkOutTime, date) : 'N/A'}
          </div>
          <br />
          <div>
              Total Hours Worked: {totalHours > 0 ? `${totalHours} hours` : 'N/A'}
          </div>
        </div>
      </div>
    </div>
