...
const LeaveReport = () => {
  ...
  const { leave_id } = useParams(); // Get leave_id from route
  ...
  // Filter leave data based on leave_id if it exists
  const filteredLeaveData = leave_id ? leaveData.filter((item) => item.id === leave_id) : leaveData;
